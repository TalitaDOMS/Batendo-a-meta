#Importa os módulos necessário para o Flask funcionar
...
if not os.path.exists(ARQUIVO):
    ...
    ws = wb.active #Selecionando a plan. ativa do projeto
...
def salvar():
    ...
    #CAPTURA A INFO. DA CX. DO FORM. CONVERTE PARA FLOAT E ATRIBUI À
    #VARIÁVEL
...
@app.route('/analisar')
def analisar():
    #Pegar o parâmetro do nome do func. enviado como parâmetro pra url
    nome_param = request.args.get('nome')
    wb = load_workbook(ARQUIVO)
    ws = wb.active
    #Loop for: percorre todas as linhas da plan. a partir da 2ªlinha
    #(Pois a 1ª linha é cabeçalho) values_only retorna apenas os valores
    for linha in ws.iter_rows(min_row=2,values_only=True):
        nome, vendas, meta = linha
        # a variável linha sempre receberá três valores, ex:
        # linha = ('Ana', 45, 50) na linha de codigo acima, estamos
        # atribuindo cada elemento da linha a uma variável na sequencia
        # as variaveis nome, vendas e meta recebem Ana, 45, 50
    ...
    if nome == nome_param:
        ...
        #Se a meta for batida bonus recebe o resultado do cálculo de
        # 15% do valor das venda, caso contrário receberá 0
        bonus = round(vendas * 0.15, 2) if meta_batida else 0
        #Exibir a tela resuldado com as infomações dos calculos
